...
document.querySelector("#recipe__like").addEventListener("click",likeHendler);
function likeHendler() {
    user = document.querySelector("#user_id").value;
    recipe = document.querySelector("#recipe_id").value;
    $.ajax({
        url:"http://localhost/recipesSite/PHP/logic/likes.php",
        method:"post",
        dataType:"json",
        data:{
            submit:"ok",
        ...
        success :function (data) {
            if(data.odg == "liked"){
                document.querySelector("#recipe__like").src= "IMG/like_full.png";
            }else{
                document.querySelector("#recipe__like").src = "IMG/like_outline.png";
            }
